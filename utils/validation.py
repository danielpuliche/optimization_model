def validar_entrada(totalNodes: int, linkCost: float, reliabilityByNodeType: list | dict) -> None:
    ...
    """
    if totalNodes < 4:
        raise ValueError(f"El número de nodos debe ser al menos 4. Se recibió: {totalNodes}")
    if linkCost <= 0:
        raise ValueError(f"El costo de un enlace debe ser mayor a 0. Se recibió: {linkCost}")
    ...
    if isinstance(reliabilityByNodeType, list):
        if not all(isinstance(value, (int, float)) and 0 <= value <= 1 for value in reliabilityByNodeType):
            raise ValueError("Todos los valores en reliabilityByNodeType deben ser números entre 0 y 1.")
